...
class Polygon:
    ...
    def iterateur_ordonnees(self):
        """
        itere sur les ordonnees des sommets
        du polygone
        """
    ...
    def segments(self):
        ...
        """
        segments = map(Segment, couples(self.points))
        return filter(lambda x:x.length() != 0, segments)
    ...
    def divide_areas(self):
        """
        divise le polygone en partitions selon les ordonnees
        des sommets du polygone
        """
        ordonnees = sorted(list(set(self.iterateur_ordonnees())))
        areas = list(couples(ordonnees))
        return areas[:-1]
